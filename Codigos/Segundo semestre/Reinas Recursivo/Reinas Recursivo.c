...
int resolverNReinasRecursivo(int tablero[N][N], int fila, int *soluciones) {
    // Si se ha colocado una reina en cada fila, se ha encontrado una solución
    if (fila == N) {
        (*soluciones)++;
        printf("Solucion %d:\n", *soluciones);
        imprimirTablero(tablero);
        return 0; // Continuar buscando más soluciones
    }

    int solucionEncontrada = 0;  // Variable para rastrear si se encuentra una solución en la llamada actual
